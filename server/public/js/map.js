...
async function predict() {
    ...
    console.log(prediction[max].className)

    axios.post("http://localhost:3000/map/food",{
        "food":prediction[max].className
    }).then((response)=>{
        var foodrest = response.data;
        console.log(foodrest.length)

        $("#main_pic").attr("src",foodrest[0].food_img);
        $('.food-content').text(foodrest[0].food_content);
    })
